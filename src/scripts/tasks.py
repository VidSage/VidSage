...
from scripts.utils import read_json_file, write_json_file, preprocess_video, extract_frames_fixed, reduce_resolution, base64_encode_frames
from scripts.videoObjects import VideoFile, Segment, VideoSummary, Story, Scene





def generate_summaries(input_json_path: str, output_json_path: str, task_id: str, llm_client: AzureOpenAI, LLM_IMG_LIMIT = 50) -> bool:
    ...
    for file in files:
        ...
        output_vid_path = f'{temp_dir}/{file["name"]}'
    ...
    for file in files:
        input_vid_path = file['absolutePath']
        frames = extract_frames_fixed(input_vid_path, temp_dir)
        ...
        for scene in scene_sec:
            ...
            for i in range(startTimeSec, endTimeSec, LLM_IMG_LIMIT):
                ...
                for frame in (encoded_frames[i:i+LLM_IMG_LIMIT] if i+LLM_IMG_LIMIT < endTimeSec else encoded_frames[i:endTimeSec]):
...
def generate_storyline(input_json_path: str, output_json_path: str, task_id: str, llm_client) -> bool:
    ...
    for _ in range(1):
      try:
          response = llm_client.beta.chat.completions.parse(
              model="TOFILL", #TODO: fill in the model
    ...
    for scene in out_story.scenes:
        ...
        storyline.append({
            ...
            "srcFile": {
                "absolutePath": output_vid_path,
                "name": f"{filename}_{scene.start}_{scene.end}"
            }
        })
...
def generate_video(input_json_path: str, output_video_path: str, task_id: str) -> bool:
    ...
    with open(f'{tmp_folder}/input.txt', 'w') as f:
        for video in video_list:
            f.write(f"file '{video}'\n")

    os.system(f'ffmpeg -f concat -safe 0 -i {tmp_folder}/input.txt -c copy {output_video_path}')
